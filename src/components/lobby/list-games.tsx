                ...
                {props.games?.map((game, i) => (
                    <li key={i} className=" mb-2" onClick={() => props.onSelect(game)}>
                        <div id='map-game' className="select-none cursor-pointer bg-map rounded-3xl flex flex-1 items-center p-4 shadow-2xl">
                            <div className="flex flex-col border-black border-2 rounded-md w-10 h-10 bg-blue-Dark bg-opacity-75 justify-center items-center mr-4">
                                <img alt="Character" src={game.players.find(player => player.user.id === authUser.id).character.img} />
                            </div>
                            <div className="flex-1 pl-1">
                                ...
                                <div className="font-iceland text-white text-sm">Pdv restant: 100</div>
